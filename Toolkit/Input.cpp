...
namespace toolkit::input {
    ...
    BOOL Down(const UINT action) {
        return keys[action];
    }

    BOOL Pressed(const UINT action) {
        return keys[action] && !previous_keys[action];
    }

    BOOL Released(const UINT action) {
        return !keys[action] && previous_keys[action];
    }

    BOOL MouseDown(const UINT action) {
        return mouse_keys[action];
    }

    BOOL MousePressed(const UINT action) {
        return mouse_keys[action] && !previous_mouse_keys[action];
    }

    BOOL MouseReleased(const UINT action) {
        return !mouse_keys[action] && previous_mouse_keys[action];
    }
}